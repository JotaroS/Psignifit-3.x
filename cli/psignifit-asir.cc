...
int main ( int argc, char ** argv ) {
	...
	if (verbose) {
		...
		std::cerr << "number of asir samples: " << nsamples << "\n";
	...
	while ( fname != "" ) {
		...
		PsiIndependentPosterior posterior = independent_marginals ( pmf, data );
		// Sample
		if ( verbose ) {
		...
		if (nsamples>0) {
			mcmc_list = new MCMCList ( sample_posterior ( pmf, data, posterior, nsamples ) );
			sample_diagnostics ( pmf, data, mcmc_list );
		} else {
			return -1;
		}
